...
uint8_t line_step = 48;  //数据间的间隔为两行，line_now加line_step即为下一数据对应的坐标
uint8_t line_beg = Line2;  //起始行为第3行
uint8_t line_end = Line7;  //末尾行为第8行
uint8_t line_now = Line2;	   //当前行对应坐标

u8 Selcct_Up,Selcct_Down;

__IO u32 uwTick_Lcd_Gundong;
...
void Line_change_page1(void)//LCD更新显示
{
	char str1[30];//用于存储当前行显示数据
	char str2[30];//用于存储上一行显示数据
    
    sprintf(str1,str_page1[line_now/line_step - 1]);
    if(Selcct_Down == 1)
    {
        sprintf(str2,str_page1[line_now/line_step - 2]);
    }
    if(Selcct_Up ==1)
    {
        sprintf(str2,str_page1[line_now/line_step]);
    }
    ...
    if(Selcct_Down)
    {
        ...
        {
            LCD_DisplayStringLine(line_end,str_page1[2]);//上一行恢复
        }  
		else
		{
			LCD_DisplayStringLine(line_now - line_step,(uint8_t *)str2);
		}
         Selcct_Down = 0;
    }  
    else if(Selcct_Up)
    {
        ...
        {
            LCD_DisplayStringLine(line_beg,str_page1[0]);//上一行恢复
        }
		else
		{
			LCD_DisplayStringLine(line_now + line_step,(uint8_t *)str2);
...
void Line_change_page5(void)//LCD更新显示
{
	char str1[30];//用于存储当前行显示数据
	char str2[30];//用于存储上一行显示数据
    
    sprintf(str1,str_page5[line_now/line_step - 2]);
    if(Selcct_Down == 1)
    {
        sprintf(str2,str_page5[line_now/line_step - 3]);
    }
    if(Selcct_Up ==1)
    {
        sprintf(str2,str_page5[line_now/line_step - 1]);
    }
    ...
    if(Selcct_Down)
    {
        if(line_now == line_beg)
        {
            LCD_DisplayStringLine(line_end,str_page5[5]);//上一行恢复
        }  
		else
		{
			LCD_DisplayStringLine(line_now - line_step,(uint8_t *)str2);
    ...
    else if(Selcct_Up)
    {
        if(line_now == line_end)
        {
            LCD_DisplayStringLine(line_beg,str_page5[0]);//上一行恢复
        }
		else
		{
			LCD_DisplayStringLine(line_now + line_step,(uint8_t *)str2);
...
void LCD_DisplayString(u8 Line,u16 column, u8 *ptr)
{
	u32 i = 0;
	while ((*ptr != 0) && (i < 20 - column))	
	{
...
void LCD_RunString(u8 Line, u8 *ptr,u16 time)
{
	if(uwTick -uwTick_Lcd_Gundong < time) return;
	 uwTick_Lcd_Gundong= uwTick;
	
	static u8 j = 20;

	LCD_DisplayString(Line8,0,(u8 *)"                    ");
	LCD_DisplayString(Line8,j,ptr);
	j--;
}
